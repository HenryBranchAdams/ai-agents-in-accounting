...
Edition `2026-09-21.6` reconciles the catalog coverage and review narrative through the accepted isolated preparation/finalization path. Its generated record-change list is empty. All .4, .5 and .6 snapshots/releases are retained. The earlier .4 files are not overwritten or presented as a successful complete release verification. The .5 change history compares to .4 as its immediate predecessor. Every pre-existing release and historical snapshot is preserved. A passing build is not public deployment.
...
Preserve both `.4` and `.5` historical bytes. The final research edition must follow `.6`. Recheck source identity conflicts, combined retrieval/exports, graph density, page and browser behavior, artifact sizes and release timings against the eventual combined corpus. Existing CI or review does not prove those future inputs. Native browser journeys and visual inspection have passed for the earlier reviewed input, as recorded in PR #181. Renew full CI for each final revision; combined-release verification and publication remain pending.


The combined-release verification adds pinned Playwright Chromium (1.63.0) and a reusable production-server harness to the complete native test inventory. The normal verification phase runs four browser configurations: 1440×1000 and 390×844, each with JavaScript enabled and disabled. The flow covers library search, the collection, FO-13/22/27, source records, private context, Back/Forward, keyboard disclosure, viewport overflow and request/console health. Browser installation occurs in the existing CI job; no separate app build or test exclusion is introduced. Screenshots and a source/lockfile/build/browser receipt are retained under ignored `outputs/browser/` in a small browser artifact so visual review does not require downloading the much larger reconstruction artifact. This adds measurable browser setup and runtime cost, to be included in final integrated performance reporting.

Browser plugin is unavailable in the current session. Local listeners and Chromium are denied by the environment, so no escalation or skipped-test substitute is used; actual browser execution is established by hosted runs. The first hosted run passed all four journeys and its desktop/mobile screenshots were visually inspected; the same run found two unrelated checks, since corrected. Exact revisions, failed attempts and renewed run outcomes are recorded in PR #181. A passing browser test still does not supply human usability acceptance.
