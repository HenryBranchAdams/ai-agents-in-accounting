...
Browser plugin is unavailable in the current session. Local listeners and Chromium are denied by the environment, so no escalation or skipped-test substitute is used; actual browser execution must be established by the hosted run. Screenshots require visual inspection after retrieval. A passing browser test still does not supply human usability acceptance.
