...
The unchanged accepted source package and original baseline inventory are integrated as edition2026-09-19.12405 after snapshot-storage edition12404. Run `node scripts/integrate-aa-i123.mjs --current` for this recorded integration; the default mode continues to reproduce the original assigned-base and corrected-candidate behavior. Current mode requires the accepted inventory and checks the input edition before writing. The nine question objects, accounting examples, source rights and primary provenance retain their source-head meanings. All preceding snapshots and releases, including original entity editions18.123 and19.1231, remain preserved. Acceptance of the original source package and acceptance of this integration are separate.

The retained entity and extractive FAS143 source IDs identify the same original publisher PDF and retain their separate bounded review scopes. An explicit same-document alias routes future imports to the existing extractive ID without deleting either record, changing existing references, or overwriting primary provenance or rights. Counts describe source records, not distinct publisher documents. Undeclared URL collisions remain errors.
