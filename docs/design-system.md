...
One `npm run build` compiles Tailwind, builds the content-hashed navigation asset, generates all corpus downloads and their hashes, packages the current source ZIP, and bundles the worker. CSS URLs have a content hash. The worker and local server serve the same navigation asset; routes remain GET/HEAD/OPTIONS only. Never patch generated downloads manually.
