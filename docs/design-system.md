...
## Reading search primitives (issue 175)

Command, Dialog and Popover were inspected with CLI 4.21.0 `info`, `docs`, `view`, `add --dry-run` and `add --diff`, then installed from the existing official Radix registry. Command adds the locked cmdk dependency. The existing MIT notice remains applicable. Imports use the project's `cn` helper; the dialog uses the semantic overlay token, equivalent scale-based centering and horizontal insets, and Command's 300px list maximum uses `max-h-75`. No lint rules were suppressed. The search palette composes Dialog and Command directly to keep accessible title and description inside the dialog and disable cmdk filtering for canonical server order.

The visible Search anchor remains `/library` before enhancement. A small navigation handler imports the palette only on deliberate opening. esbuild splitting shares React/Radix chunks while entry metadata identifies the navigation script. The document carries its corpus edition, and the palette rejects responses from a different edition. Same-origin `connect-src` permits this read-only request; other script and security restrictions remain. The header wraps at narrow widths, and the full navigation appears at the extra-large breakpoint so Search and Menu remain reachable at intermediate sizes.

Popover has one explicit `size="evidence"` layout variant. Its viewport-bounded width/height and scrolling are owned by the primitive and the semantic CSS layer; callers do not restyle the component. Material rights and limitations are never line-clamped. Print hides only enhancement controls and popups, leaving the ordinary evidence text and source links. No new lint exceptions were added.

## Connection exploration (issue 171)

The focused connection view is a separate bounded island. Its server-rendered List and native GET links remain available before JavaScript and on failure. Only explicit Graph mode loads its client module, Cytoscape and Resizable. It imports no canonical corpus or whole connection index. Shared React/Radix chunks retain exact entry mapping and the generated worker asset allowlist.
