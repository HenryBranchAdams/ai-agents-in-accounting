...
function markdownHeader(title: string, summary: string, count: number) {
  return [
    ...
    `> ${summary}`,
    "",
    `Records: ${count}. Corpus snapshot: ${corpusReviewedAt}. Schema: 1.0.`,
    "",
    domainRightsNotice,
    "",
...
export function renderAuthorityLevelsMarkdown(records: AuthorityLevel[] = authorityLevels) {
  ...
  lines.push(
    ...
    "",
    "| Stable ID | Level | Pattern classification | Entry condition | Decision owner | Permitted effect | Accounting example | Example classification | Stop when |",
    "|---|---|---|---|---|---|---|---|---|",
    ...guide.execution_comparison.map((item) => `| \`${item.id}\` | ${item.level_id} | ${item.evidence_classification} | ${tableCell(item.entry_condition)} | ${tableCell(item.decision_owner)} | ${tableCell(item.permitted_effect)} | ${tableCell(item.accounting_example)} | ${item.accounting_example_classification} | ${tableCell(item.stop_when)} |`),
    "",
