...
The [bank-reconciliation workflow](https://accounting-agents.madebyhenry.chatgpt.site/workflows/record-to-report/wf-r2r-bank-reconciliations) pilots a one-minute brief that lets a reader assess fit, prerequisites, authority, the top check and failure, expected review artifact, and supervised-pilot conditions before opening the full record. The same stable brief is included in the workflow [Markdown](https://accounting-agents.madebyhenry.chatgpt.site/api/v1/workflows/wf-r2r-bank-reconciliations?format=markdown) and [JSON](https://accounting-agents.madebyhenry.chatgpt.site/api/v1/workflows/wf-r2r-bank-reconciliations) projections. It is an implementation pattern under maintainer review, not a professional conclusion.
