...
const integrateIntoNewerCorpus = currentIntegration || process.argv.includes("--integrate-into-newer-corpus");
...
if (currentIntegration) {
  const version = "2026-09-19.12406";
  assert.ok(["2026-09-19.12405", version].includes(catalog.corpus_version), "Refuse an unrecognized tax integration edition before writes");
  if (catalog.corpus_version !== version) {
    catalog.coverage_note += ` Edition${version} integrates five bounded US tax questions, thirteen original-source records and an original synthetic tax-boundary example while preserving prior research.`;
    catalog.review_note += ` Edition${version} integrates accepted tax source62ec417d after preserved entity edition2026-09-19.12405. Original tax source edition2026-09-19.1 remains historical evidence. Taxpayer facts, current consolidated Topic740, professional review, external rights and empirical limits remain unresolved where recorded; no filing or deployment is authorized.`;
  }
  catalog.corpus_version = version;
  catalog.updated_at = maxDate(catalog.updated_at, date);
}
...
const foundationMarker = {
  ...
  package_id: packet.package_id,
  package_version: packet.version,
  // Retain the edition that first integrated this source package on replay.
  corpus_version: foundations.aa_i122?.corpus_version || catalog.corpus_version,
...
if (currentIntegration) write("data/catalog.json", catalog);
