...
try{
 ...
 for(const[name,viewport]of[['desktop',{width:1440,height:1000}],['mobile',{width:390,height:844}]]){
  ...
  try{
   ...
   const response=await page.goto(origin+'/records/'+pilot.id);assert.ok(response.headers()['content-security-policy'].includes("connect-src 'self'"));assert.equal(response.headers()['x-content-type-options'],'nosniff');
   ...
   await page.goto(origin+'/library?q=bank&kind=source');assert.ok((await page.locator('main').innerText()).includes('source'));assert.equal(new URL(page.url()).searchParams.get('q'),'bank');
