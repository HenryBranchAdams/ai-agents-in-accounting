...
const entries={navigation:'src/client/navigation.tsx',outline:'src/client/reading.ts',preview:'src/components/evidence-preview.tsx',search:'src/components/search-palette.tsx',graph:'src/client/connections.tsx'};
...
const report={
 ...
 scenarios_gzip_bytes:{navigation:bytes(sets.navigation),navigation_and_outline:bytes(union(sets.navigation,sets.outline)),edited_brief_with_preview:bytes(reading),edited_brief_then_search:bytes(union(reading,sets.search)),graph:bytes(union(sets.navigation,sets.graph))},
 graph_modules_in_ordinary_reading:[...reading].flatMap(file=>Object.keys(metadata.outputs[file].inputs)).filter(file=>/(?:connections|connection-canvas|cytoscape|react-resizable-panels)/u.test(file)),
};
