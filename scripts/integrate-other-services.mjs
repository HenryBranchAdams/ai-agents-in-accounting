...
export const packetFile='data/research/other-services-2026-09-19.json';
const rootOf=()=>path.resolve(process.env.OTHER_SERVICES_ROOT||process.cwd());
...
export function applyToRoot(root=rootOf(),{dryRun=false}={}){
  ...
  assert.equal(catalog.corpus_version,pkg.current_corpus_version,'Refuse unexpected other-services integration edition before writes');
...
function copyPacketAndHelper(sourceRoot,targetRoot){
  for(const file of [packetFile,'scripts/integrate-other-services.mjs']){const dest=path.join(targetRoot,file);fs.mkdirSync(path.dirname(dest),{recursive:true});fs.copyFileSync(path.join(sourceRoot,file),dest);}
...
export async function runAppliedValidation(sourceRoot=rootOf()){
  const pkg=read(sourceRoot,packetFile);const before=execFileSync('git',['-C',sourceRoot,'status','--porcelain'],{encoding:'utf8'});const temp=fs.mkdtempSync(path.join(os.tmpdir(),'aa-i115-applied-'));
  try{
...
if(import.meta.url===pathToFileURL(process.argv[1]||'').href){
  ...
  else fail('Usage: node scripts/integrate-other-services.mjs --describe|--dry-run|--apply|--validate-applied');
}
