...
const preserveVersion = (current, incoming) => {
  if (!current) return incoming;
  if (!incoming) return current;
  return compareVersion(current, incoming) > 0 ? current : incoming;
};
const preserveDate = (current, incoming) => current && current > incoming ? current : incoming;
const sourceRights = {
  metadata: "CC0-1.0",
  content: "CC-BY-4.0",
...
for (const [label, current, expected] of [["question registry", registry.question_set_version, packageData.version], ["assessment coverage", assessments.assessment_version, assessmentData.assessment_version], ["mapping overrides", overrides.mapping_version, packageData.version]]) {
  if (!integrateIntoNewerCorpus && current && compareVersion(current, expected) > 0) throw new Error(`${label}: newer version ${current} would be overwritten by ${expected}`);
}
if (!integrateIntoNewerCorpus && registry.reviewed_at > date) throw new Error(`question registry: newer review date ${registry.reviewed_at} would be overwritten by ${date}`);
if (!integrateIntoNewerCorpus && overrides.updated_at > date) throw new Error(`mapping overrides: newer review date ${overrides.updated_at} would be overwritten by ${date}`);
for (const [file, coverage] of [["data/coverage/subsector-profiles.json", profileCoverage], ["data/coverage/subsector-screening.json", screeningCoverage]]) {
  if (!integrateIntoNewerCorpus && coverage.corpus_version && compareVersion(coverage.corpus_version, catalog.corpus_version) > 0)
...
for (const family of packageData.families) {
  ...
  if (integrateIntoNewerCorpus && guide.data?.version && compareVersion(guide.data.version, packageData.version) > 0)
    throw new Error(`${guide.id}: newer guide version ${guide.data.version} would be overwritten by ${packageData.version}`);
...
registry.question_set_version = preserveVersion(registry.question_set_version, packageData.version);
registry.corpus_version = catalog.corpus_version;
registry.reviewed_at = preserveDate(registry.reviewed_at, date);
...
assessments.assessment_version = preserveVersion(assessments.assessment_version, assessmentData.assessment_version);
overrides.mapping_version = preserveVersion(overrides.mapping_version, packageData.version);
overrides.updated_at = preserveDate(overrides.updated_at, date);
