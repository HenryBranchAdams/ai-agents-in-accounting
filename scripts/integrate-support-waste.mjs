...
export const packetFile='data/research/support-waste-2026-09-19.json';
export function applyToRoot(root,{expectedVersion,dryRun=false,currentMode=false}={}) {
  ...
  const version='2026-09-19.12422';
  if(currentMode)assert.ok(['2026-09-19.12421',version].includes(catalog.corpus_version),'Refuse unexpected support/waste current integration edition before writes');
  else assert.equal(catalog.corpus_version,expectedVersion||packet.current_corpus_version,'Refuse unexpected support/waste integration edition before writes');
  ...
  const add=(rows,row,label)=>{const old=rows.find(r=>r.id===row.id);assert.ok(!old||isDeepStrictEqual(old,row),`Support/waste preflight conflict: ${label}:${row.id}`);if(!old)rows.push(structuredClone(row));};
  for(const row of [...packet.sources,...packet.records]){
    assert.ok(stage.has(`data/corpus/${row.kind}.json`),`Unsupported kind ${row.kind}`);
    const old=index.get(row.id);assert.ok(!old||isDeepStrictEqual(old,row),`Support/waste preflight conflict: record:${row.id}`);
    if(row.kind==='source'){
      assert.ok(!urls.has(row.source_url)||urls.get(row.source_url)===row.id,`Support/waste source URL identity conflict: ${row.id}`);
  ...
  for(const row of [...packet.records,...packet.sources]){
    for(const id of row.source_ids)assert.equal(index.get(id)?.kind,'source',`Missing support/waste source ${id}`);
    for(const id of row.related_ids)assert.ok(index.has(id),`Missing support/waste related record ${id}`);
  }
  ...
  for(const [id,row] of Object.entries(packet.mapping_overrides)){const old=mappings.records[id];assert.ok(!old||isDeepStrictEqual(old,row),`Support/waste preflight conflict: mapping:${id}`);mappings.records[id]=row;}
  stage.set(file,mappings);
  if(currentMode){
    const note=` Edition ${version} adds selected US staffing, administrative support, waste and remediation questions with separate synthetic entity worksheets.`;
    const review=` Edition ${version} retains dated source limits, separate service, tax and environmental roles, and all earlier source rights and history.`;
    ...
    const file='data/research-questions.json',fixtures=read(file);for(const row of read('data/research/support-waste-retrieval-2026-09-19.json'))add(fixtures,row,'retrieval');stage.set(file,fixtures);
  }
  let changed=0;
