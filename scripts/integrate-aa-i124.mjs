...
const stage = (p, v) => staged.set(p, JSON.stringify(v, null, 2) + '\n');
const packet = read('data/research/institutional-regulated-124.json');
const catalog = read('data/catalog.json');
assert.ok(['2026-09-18.1', packet.package_version, packet.corpus_edition].includes(catalog.corpus_version),
  'Refuse integration into a different edition; coordinator must explicitly integrate the additive package.');
...
registry.question_set_version = packet.package_version;
registry.corpus_version = packet.corpus_edition;
registry.reviewed_at = packet.reviewed_at;
assessments.assessment_version = packet.package_version;
...
catalog.corpus_version = packet.corpus_edition;
...
for (const path of ['data/coverage/subsector-profiles.json','data/coverage/subsector-screening.json']) {
  ...
  value.corpus_version = packet.corpus_edition;
  stage(path, value);
}
...
console.log(`Integrated ${packet.issue_id}: ten partial questions; prior records and history preserved.`);
