...
const version = currentIntegration ? '2026-09-19.12405' : p.version;
if (currentIntegration) {
  const input = read('data/catalog.json').corpus_version;
  assert.ok(['2026-09-19.12404', version].includes(input), 'Refuse an unrecognized current integration edition before writes.');
  assert.ok(fs.existsSync('data/research/aa-i123-inventory.json'), 'Current integration requires the accepted baseline inventory.');
}
...
const retainedAliases = currentIntegration ? { src_aa_i123_fas143: 'src_us_extractive_fas143_historical' } : {};
if (currentIntegration) {
  const aliases = read('data/research/source-aliases.json');
  for (const [id, targetId] of Object.entries(retainedAliases)) {
    const proposal = p.sources.find(source => source.id === id);
    const target = sources.find(source => source.id === targetId);
    assert.equal(target?.source_url, proposal?.url, `Retained source alias must identify the same publisher document: ${id}`);
    assert.ok(!aliases[id] || aliases[id] === targetId, `Conflicting source alias: ${id}`);
    aliases[id] = targetId;
  }
  save('data/research/source-aliases.json', aliases);
}
...
for (const s of p.sources) {
  ...
  if (s.reuse) {
    ...
    assert.ok(sources.filter(r=>r.source_url===s.url && r.id!==s.id).every(r=>retainedAliases[s.id]===r.id),`Duplicate publisher URL ${s.url}`);
...
registry.question_set_version=version;registry.corpus_version=version;registry.reviewed_at=date;
...
for (const file of ['data/coverage/subsector-profiles.json','data/coverage/subsector-screening.json','data/coverage/research-criteria.json']) {
  // The current coverage generator does not store an edition on this contract.
  // Preserve the original source applicator's historical behavior only there.
  if (file.endsWith('research-criteria.json')) {if (!currentIntegration) criteria.corpus_version=version;continue;}
  const generated=read(file);generated.corpus_version=version;save(file,generated);
}
const catalog=read('data/catalog.json');
if (currentIntegration) {
  if (catalog.corpus_version !== version) {
    catalog.coverage_note += ` Edition${version} integrates nine bounded entity-event questions, nine original-source records and one synthetic reconciliation example while preserving prior industry and foundation research.`;
    catalog.review_note += ` Edition${version} integrates accepted entity source038e5932 after preserved storage edition2026-09-19.12404. Original entity releases2026-09-18.123 and2026-09-19.1231 remain historical evidence; no current-authority, rights, professional-review, empirical or deployment claim is upgraded.`;
  }
  catalog.corpus_version = version;
  catalog.updated_at = date;
} else if (catalog.corpus_version!==p.version) {
...
console.log(`Integrated ${p.id}: ${p.questions.length} questions; ${families.length} guides; ${version}. Run coverage:map, coverage:snapshot and check.`);
