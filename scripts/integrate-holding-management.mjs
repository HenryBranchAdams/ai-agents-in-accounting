...
export function applyToRoot(root,{expectedVersion,dryRun=false}={}) {
  ...
  const add=(rows,row,label)=>{const old=rows.find(r=>r.id===row.id);assert.ok(!old||isDeepStrictEqual(old,row),`Holding preflight conflict: ${label}:${row.id}`);if(!old)rows.push(structuredClone(row));};
  for(const row of [...packet.sources,...packet.records]){
    assert.ok(stage.has(`data/corpus/${row.kind}.json`),`Unsupported kind ${row.kind}`);
    const old=index.get(row.id);assert.ok(!old||isDeepStrictEqual(old,row),`Holding preflight conflict: record:${row.id}`);
    if(row.kind==='source'){
      assert.ok(!urls.has(row.source_url)||urls.get(row.source_url)===row.id,`Holding source URL identity conflict: ${row.id}`);
  ...
  for(const row of [...packet.records,...packet.sources]){
    for(const id of row.source_ids)assert.equal(index.get(id)?.kind,'source',`Missing holding source ${id}`);
    for(const id of row.related_ids)assert.ok(index.has(id),`Missing holding related record ${id}`);
  }
