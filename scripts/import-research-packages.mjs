...
for (const batch of batches) for (const s of batch.sources || []) {
  const sourceScope = scopedIds(batch, "source");
  if (sourceScope && !sourceScope.has(s.id)) continue;
...
for (const original of batches) {
  ...
  for (const p of packages) {
    ...
    const familyScope = scopedIds(batch, "family");
    const packageInScope = !familyScope || family_ids.some(familyId => familyScope.has(familyId)) || familyScope.has(id);
