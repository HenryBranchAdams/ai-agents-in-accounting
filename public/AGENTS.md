...
## Roadmap research edition

`naics` is an exact NAICS-US 2022 code and `question_family` is an exact topology family ID. Neither inherits from broader or narrower codes. Find a directly associated guide, then follow its `source_ids` without carrying an industry filter onto shared accounting authorities. `research_review` exposes review scope and section hints; `data.research_questions` passages retain named answers, locators, controls and gaps. Read all paginated passages required for the question.

The coverage response includes a `research` view with named questions, the selected subsector profile, screening decisions and leaf exceptions. A detailed-industry selection labels parent screening as context and returns that leaf's own review. The six research downloads are `research-questions.json`, `research-criteria.json`, `subsector-profiles.json`, `subsector-screening.json`, `industry-exception-reviews.json` and `classification-relationships.json`. Their contract is `/schemas/research.schema.json`. Source and editorial disposition ledgers are also downloadable. These exports and the source ZIP share the build manifest.

The declared worklist includes 178 named questions, 62 broader family questions, 5,577 open applicable/conditional screening pairs, and 616 additional leaf-question entries. These overlapping views must not be summed into a percentage of accounting knowledge. Keep 375 role-specific exclusions and their reopening triggers visible. All 1,012 leaves have classification-based exception dispositions, which do not certify their accounting treatment. Preserve the distinction between substantive-excerpt, abstract/landing and unresolved source checks. No whole family or industry is sufficient, no professional review has been performed, and synthetic examples are not observed deployments.
