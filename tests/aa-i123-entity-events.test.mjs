...
test('AA-I123 source rights and same-build export/archive membership remain exact',()=>{
  ...
  assert.equal(manifest.corpus_version,meta.corpus_version);
  ...
  const release=read(`data/releases/${meta.corpus_version}/corpus.json`);
  ...
  const snapshot=readSnapshotHistory().snapshots.find(s=>s.id===meta.corpus_version);
  assert.deepEqual(snapshot.summary,read('dist/client/downloads/coverage.json').summary);
  const prior=readSnapshotHistory().snapshots.find(s=>s.id==='2026-09-18.123');
