...
test("AA-I127 build exports and source export include the new canonical records", () => {
  ...
  assert.equal(exported.exported_record_count, 1121);
...
test("AA-I127 derived research denominators agree across criteria, runtime analytics, and the latest snapshot", () => {
  ...
  assert.equal(latest.id, "2026-09-19.3");
...
test("AA-I127 shared assessments, cited locators, and empirical source reviews are canonical and synchronized", () => {
  ...
  const namedShared = shared.filter((assessment) => namedQuestions.some((question) => question.id === assessment.named_question_id));
  assert.equal(namedShared.length, namedQuestions.length);
  assert.ok(shared.every((assessment) => assessment.industry_code === null && assessment.named_question_id));
  assert.deepEqual(new Set(namedShared.map((assessment) => assessment.named_question_id)), new Set(namedQuestions.map((question) => question.id)));
