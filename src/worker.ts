...
import { CoverageQueryError, coverageTopology } from "./coverage";
...
import { AgentError, agentError } from "./agent-errors";
import { agentJsonSchema, operationDescriptions } from "./agent-contract";
import type { AgentOperation } from "./agent-contract";
...
async function route(request: Request, env: Env) {
  ...
  if (path.startsWith("/api/v1/agent/")) {
    ...
    const { executeAgent, parseAgentQuery } = await import("./agent");
