...
async function showSearch(trigger: HTMLElement | null) {
  ...
  try {
    ...
    if (intent === searchIntent && !document.querySelector('[role="dialog"]')) openSearch(document.body.dataset.corpusVersion || "", trigger);
  } catch {
    if (intent === searchIntent) window.location.assign("/library");
  } finally { if (intent === searchIntent) opening = false; }
}
...
document.addEventListener("keydown", event => {
  if (event.key === "Escape" && opening) { searchIntent++; opening = false; return; }
